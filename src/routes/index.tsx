...
function Summary({ icon: Icon, label, value, note, tone }: { icon: typeof Radio; label: string; value: string; note: string; tone: string }) {
  return <article className="summary-card"><div className="flex items-center justify-between"><span className="label">{label}</span><Icon size={15} className={`text-${tone}`} /></div><div className="mt-3 font-mono text-2xl font-semibold">{value}</div><p className="mt-1 font-mono text-[10px] text-muted-foreground">{note}</p></article>;
}
...
function TopologyLayer({ icon: Icon, title, subtitle, items }: { icon: typeof Radio; title: string; subtitle: string; items: [string, Status][] }) { return <div className="topology-layer"><div className="mb-3 flex items-center gap-2"><Icon size={15} className="text-primary"/><div><h3 className="font-mono text-[11px] font-semibold">{title}</h3><p className="font-mono text-[9px] text-muted-foreground">{subtitle}</p></div></div><div className="space-y-2">{items.map(([name,status]) => <div className="node" key={name}><span className={`status-dot bg-${status === "online" ? "success" : status === "busy" ? "warning" : "destructive"}`}/><span className="font-mono text-[11px]">{name}</span><span className="ml-auto font-mono text-[8px] uppercase text-muted-foreground">{status}</span></div>)}</div></div>; }
...
function EdgeCard(node: typeof edgeNodes[number]) { return <article className={`panel ${node.status === "busy" ? "border-warning/40" : ""}`}><div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2"><Server size={15} className="text-primary"/><h3 className="font-mono text-sm font-semibold">{node.name}</h3></div><span className={`status-badge ${node.status === "busy" ? "text-warning" : "text-success"}`}><i className={`status-dot bg-${node.status === "busy" ? "warning" : "success"}`}/>{node.status}</span></div><div className="space-y-3"><Meter label="CPU" value={node.cpu}/><Meter label="Memory" value={node.memory}/><Meter label="Network" value={node.network}/></div><div className="mt-4 flex justify-between border-t border-border pt-3 font-mono text-[10px] text-muted-foreground"><span>Active workloads</span><span className="text-foreground">{node.workloads}</span></div></article>; }
function Meter({ label, value }: { label: string; value: number }) { const width = Math.round(value / 10) * 10; return <div><div className="mb-1.5 flex justify-between font-mono text-[10px] text-muted-foreground"><span>{label}</span><span className={value > 75 ? "text-warning" : "text-foreground"}>{value}%</span></div><div className="h-1.5 overflow-hidden bg-muted"><div className={`h-full bg-${value > 75 ? "warning" : "primary"} w-${width}`}/></div></div>; }
